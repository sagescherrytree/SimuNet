...
export class CopyToPointsNode extends Node implements IGeometryModifier {
  ...
  private extractedPositions?: number[][];
  ...
  applyModificationMultiple(inputs: GeometryData[]): GeometryData | undefined {
    ...
    let pointAttributeBuffer = tgt.pointAttributeBuffer;

    // GPU stuffs.
    const gpu = GPUContext.getInstance();

    if (!pointAttributeBuffer) {
      console.log("CopyToPointsNode: Creating default point attribute buffer");

      // Create default attributes: pscale=1, scale=(1,1,1), orient=(0,0,0,1)
      const attribCount = this.vertexCountTgt;
      const attribStride = 9; // pscale(1) + pad(1) + scale(3) + orient(4) = 9 floats
      const defaultAttribs = new Float32Array(attribCount * attribStride);

      for (let i = 0; i < attribCount; i++) {
        const base = i * attribStride;
        defaultAttribs[base + 0] = 1.0; // pscale
        defaultAttribs[base + 1] = 0.0; // pad0
        defaultAttribs[base + 2] = 1.0; // scale.x
        defaultAttribs[base + 3] = 1.0; // scale.y
        defaultAttribs[base + 4] = 1.0; // scale.z
        defaultAttribs[base + 5] = 0.0; // orient.x
        defaultAttribs[base + 6] = 0.0; // orient.y
        defaultAttribs[base + 7] = 0.0; // orient.z
        defaultAttribs[base + 8] = 1.0; // orient.w
      }
      pointAttributeBuffer = gpu.device.createBuffer({
        size: defaultAttribs.byteLength,
        usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
      });
      gpu.device.queue.writeBuffer(pointAttributeBuffer, 0, defaultAttribs);
    }

    if (
      this.vertexCountSrc === 0 ||
      this.indexCountSrc === 0 ||
      this.vertexCountTgt === 0
    ) {
    ...
    this.setupComputePipeline(
      ...
      outputVertexBuffer,
      outputIndexBuffer,
      this.cpyToPtsUniformBuffer
    );
    this.SetupWireframe(
      outputIndexBuffer,
      outputWireframeBuffer,
      triangleCount
    );

    const instanceCount = this.vertexCountTgt;

    this.generateBoundingBuffersAsync(
      src,
      tgt,
      instanceCount,
      this.stride,
      tgt.pointAttributeBuffer
    )
      .then(() => {
        ...
        if (this.geometry) {
          ...
          // add extracted positions to geometry
          this.geometry.instancePositions = this.extractedPositions;
          this.geometry.pointCount = instanceCount;
    ...
    const workgroups = Math.ceil(this.vertexCountTgt / this.workgroupSize);
    ...
    console.log("CopyToPoints output:", {
      pointCount: instanceCount,
      hasPositions: !!this.extractedPositions,
      positionCount: this.extractedPositions?.length,
    });
  ...
  setupComputePipeline(
    ...
    srcIndexBuffer: GPUBuffer,
    tgtVertexBuffer: GPUBuffer,
    pointAttributeBuffer: GPUBuffer,
    outputVertexBuffer: GPUBuffer,
    outputIndexBuffer: GPUBuffer,
    cpyToPtsUniBuffer: GPUBuffer
  ) {
    const gpu = GPUContext.getInstance();
    ...
    this.cpyToPtsComputeBindGroupLayout = gpu.device.createBindGroupLayout({
      ...
      entries: [
        ...
        }, // target vertices
        {
          ...
          visibility: GPUShaderStage.COMPUTE,
          buffer: { type: "read-only-storage" },
        }, // point attribute buffer from attrib random node, if exists.
        {
          binding: 4,
          visibility: GPUShaderStage.COMPUTE,
          buffer: { type: "storage" },
    ...
    console.log(
      "CopyToPointsNode: pipeline created:",
      this.cpyToPtsComputePipeline
    );
  ...
  SetupWireframe(
    outputIndexBuffer: GPUBuffer,
    outputWireframeBuffer: GPUBuffer,
    ...
    gpu.device.queue.writeBuffer(
      this.mergeWireframeUniformBuffer,
      0,
      wireframeUniformData
    );
  ...
  async generateBoundingBuffersAsync(
    ...
    this.extractedPositions = positions;
    ...
    // done
    console.log(
      "CopyToPoints: bounding buffers created. instances=",
      instanceCount
    );
  }
