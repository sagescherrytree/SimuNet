...
fn worley3D( p: vec3f ) -> f32 {
    let pFloor = floor(p);
    var minDist = 1000.f;
    for (var dz = -1.f; dz <= 1.f; dz += 1.f) {
        for (var dy = -1.f; dy <= 1.f; dy += 1.f) {
            for (var dx = -1.f; dx <= 1.f; dx += 1.f) {
                let gridPoint = pFloor + vec3f(dx,dy,dz);
                let samplePoint = random3D(gridPoint) + gridPoint;

                let curDist = length(samplePoint - p);
                if (minDist > curDist) {
                    minDist = curDist;
                }
            }
